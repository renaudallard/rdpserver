...
- **Bidirectional clipboard** — MS-RDPECLIP static virtual channel, text formats (CF_UNICODETEXT).  Copy in the remote `xterm` and paste in your local clipboard; copy locally and paste in `xterm`.  `XFixesSelectSelectionInput` watches the X CLIPBOARD selection and the worker bridges to the RDP channel via the backend RPC.  Large transfers are handled end to end: inbound channel fragments are reassembled, the X selection is read past the 16 KiB property limit (including the `INCR` protocol) and written back in chunks, bounded at 4 MiB.
