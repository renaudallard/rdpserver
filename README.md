...
| Audio input / microphone (MS-RDPEAI) | partial | The worker opens the `AUDIO_INPUT` dynamic virtual channel and runs the SNDIN negotiation (Version, Formats, Open), offering PCM 16-bit stereo 44.1 kHz; the client's captured microphone PCM arrives as SNDIN Data PDUs and is forwarded to the session as `RDP_BE_AUDIO_INPUT` chunks (bounded at 64 KiB).  A client that does not redirect a microphone simply produces no data, with no effect on the session.  Offered by default; suppress with `rdpd -m`.  The session-side PulseAudio source that plays the forwarded PCM into apps is a follow-up stage. |
