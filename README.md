...
| Persistent bitmap cache (MS-RDPBCGR) | ✓ | With `rdpd -B` (off by default) the server advertises the Bitmap Cache Rev2 and host-support capability sets and ingests the client's Persistent Key List PDU at connect, pre-seeding the slots the client already holds on disk.  On the fast-path bitmap path each 64x64 tile is keyed (FNV-1a) and looked up in the per-connection cache: a hit recalls the cached tile with a MemBlt drawing order alone; a miss stores it with a Cache Bitmap Rev2 secondary order (interleaved RLE compressed, falling back to nothing larger than the raw tile) before the MemBlt blits it, both carried in one fast-path Orders update.  The three caches hold 120, 120 and 336 slots; the oldest slot is evicted round-robin when a cache fills.  This saves re-sending repeated tiles (window chrome, text, backgrounds) on the legacy bitmap path and has no effect on a GFX or AVC session, which never reaches this path.  The MemBlt, Cache Bitmap Rev2 and interleaved RLE encodings are byte-validated against the FreeRDP reference (order, bmpcache and bitmap_rle regress); a live end-to-end test is constrained because current clients negotiate the GFX path by default. |
