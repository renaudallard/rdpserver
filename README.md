...
- **Bidirectional clipboard** — MS-RDPECLIP static virtual channel, text (CF_UNICODETEXT), HTML, and image formats.  Copy in the remote `xterm` and paste in your local clipboard; copy locally and paste in `xterm`.  `XFixesSelectSelectionInput` watches the X CLIPBOARD selection and the worker bridges to the RDP channel via the backend RPC.  HTML is mapped both ways between the X `text/html` target and the registered Windows `HTML Format` (the worker adds/strips the CF_HTML `Version`/`StartFragment` envelope; the registered format name is matched in the format list and its dynamic id learned per session).  Images are carried between the X `image/bmp` target and the Windows `CF_DIB`/`CF_DIBV5` formats by a zero-dependency BMP conversion (the worker prepends or strips the 14-byte `BITMAPFILEHEADER`).  Files copied in the session (the X `text/uri-list` target) are offered to the client as a `FileGroupDescriptorW`, and their contents are streamed on demand over the `FILECONTENTS` PDUs so a file copied in the Linux session can be pasted in the client (the reverse direction, pasting a client file into the session, is in progress).  Large transfers are handled end to end: inbound channel fragments are reassembled, the X selection is read past the 16 KiB property limit (including the `INCR` protocol) and written back in chunks, bounded at 4 MiB.
...
| CLIPRDR clipboard, bidirectional, text + HTML + image formats | ✓ | `HTML Format` mapped to X `text/html`; `CF_DIB`/`CF_DIBV5` mapped to X `image/bmp`. |
