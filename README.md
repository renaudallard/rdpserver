...
| Drive / printer / serial redirection | ✓ handshake | RDPDR channel with full capability exchange, device enumeration, and per-device acknowledgment.  Drives, printers, serial/parallel ports, and smartcards announced by the client are recognized and logged.  File I/O forwarding (IRP dispatch) is the next step. |
